...
// and each oligonucleotide count needToOutput by this program.
...
// desired_Length_For_Oligonucleotides and then save it to needToOutput.
static void generate_Frequencies_For_Desired_Length_Oligonucleotides(
    ...
    if(output)
    {
        ...
        for(intnative_t output_Position=0, i=0; i<elements_Array_Size; i++){
            ...
            // Output the frequency for oligonucleotide to needToOutput.
...
// polynucleotide and then save it to needToOutput.
static void generate_Count_For_Oligonucleotide(
    ...
    // Output the count for oligonucleotide to needToOutput.
...
void KNucleotideRun(char* inputFile, int output){
    char buffer[4096];
    FILE *inputStream = fopen(inputFile, "r");
    needToOutput = output;

    // Find the start of the third polynucleotide.
    while(fgets(buffer, sizeof(buffer), inputStream) && memcmp(">THREE", buffer
            , sizeof(">THREE")-1));
    ...
    while(fgets(buffer, sizeof(buffer), inputStream) && buffer[0]!='>'){
